function [shifted, averaged, final] = shiftavg(MR_images, xcorr, ycorr)
    ...
    for i = 1:5
        sz = size(MR_images{i});
        if sz(2) < maxx
            MR_images{i} = uint8([MR_images{i} zeros(sz(1), maxx-sz(2))]);
        end
        sz = size(MR_images{i});
        if sz(1) < maxy
            MR_images{i} = uint8(transpose([transpose(MR_images{i}) zeros(sz(2), maxy-sz(1))]));
        end
    end
    ...
    final = averaged;
    final = final(:,1:end-removex);
    final = final(1:end-removey,:);
end